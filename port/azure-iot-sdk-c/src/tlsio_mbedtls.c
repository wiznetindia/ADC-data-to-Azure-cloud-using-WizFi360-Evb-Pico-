...
#define DEBUG_LEVEL 0 // 0 No debug, 1 Error, 2 State change, 3 Informational, 4 Verbose
...
#if defined(MBEDTLS_DEBUG_C)
        mbedtls_ssl_conf_dbg(&tls_io_instance->config, mbedtls_debug, NULL);
#endif
    ...
    else
    {
        ...
        if (strcmp(OPTION_TRUSTED_CERT, optionName) == 0)
        {
            ...
            else
            {
                ...
                if (parse_result > 0)
                {
                    /* This will happen if the CA chain contains one or more invalid certs, going ahead as the handshake
                     * may still succeed if the other certificates in the CA chain are enough for the authentication */
                    LogInfo("Parsing was partly successful. Number of certificates that couldn't be parsed: %d", parse_result);
                }
